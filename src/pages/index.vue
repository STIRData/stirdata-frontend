...
<script>
  export default {
    ...
    async asyncData ({ $backendApi }) {
        try{ 
           let response = await $backendApi.get('nuts?parent=https://lod.stirdata.eu/nuts/code/NO0');
    ...
    methods: {
      ...
      onSubmit(event) {
        this.$backendApi.get('nuts?parent=https://lod.stirdata.eu/nuts/code/NO0').then(response => {
          console.log(response.data);
        })
        .catch(error => {
          console.log(error);})
